		...
		if(classname == "test_report"){
			...
			for(var j=0 ; j<test_report_divs.length ; j=j+2){
				...
				if( (!stf && !endf) && pass ){
					...
					var mydate = new Date(begin_y,begin_m,begin_d);
					var mylongstart =  mydate.getTime();
					//alert("start:"+mydate);
					
					var mydate = new Date(end_y,end_m,end_d); 
					var mylongend =  mydate.getTime();
					//alert("end:"+mydate);
					
					//alert(mylongstart+"|"+mylongdate+"|"+mylongend);
					if( mylongstart > mylongdate || mylongdate > mylongend ){
				...
				if((!spdf) && pass){
					...
					if(spd_version != spdv){
						...
						/* var ff = true;
						for(var index_ = 0 ; index_ < spd_versionL ; index_++){
							var partSpdf = spd_version[index_];
							if(partSpdf.toLowerCase() != spdv[index_].toLowerCase()){
								ff = false;
								break;
							}
						}
						...
						alert(spdv.toLowerCase()+"--"+spd_version);
